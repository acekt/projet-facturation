...
/**
 * LoginPage — Page de Connexion "L'Étoile"
 * =========================================
 *
 * Refonte UI/UX : Design SaaS Enterprise & Accessibilité (Contrastes WCAG AA)
 *  - Layout centré sous forme de grande carte à bords très arrondis (`rounded-3xl`)
 *  - Structure bi-colonne (Gauche: Branding & Valeurs, Droite: Formulaire de connexion)
 *  - Contrastes renforcés : remplacement systématique des gris clairs par des teintes
 *    hautement lisibles (`text-slate-700` en mode clair, `text-zinc-300` en mode sombre).
 *  - Dégradé de texte "excellence" réajusté avec des couleurs à fort contraste (Indigo/Bleu).
 *  - Raccourci d'auto-complétion interactif pour les comptes de démonstration (Admin / Opérateur)
 *  - Typage TypeScript rigoureux (zéro `any`).
 */
...
export default function LoginPage() {
  ...
  const fillDemoCredentials = (role: 'admin' | 'operator') => {
    if (role === 'admin') {
      setUsername('admin@letoile.ga')
      setPassword('admin123')
      toast.info("Identifiants Administrateur insérés")
    } else {
      setUsername('operateur@letoile.ga')
      ...
      <div className="max-w-5xl w-full bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-[2rem] shadow-2xl overflow-hidden grid grid-cols-1 md:grid-cols-12 min-h-[620px] transition-all duration-300">
        ...
        <div className="md:col-span-5 bg-slate-100/60 dark:bg-zinc-900/50 border-r border-slate-200 dark:border-zinc-800 p-8 md:p-12 flex flex-col justify-between relative overflow-hidden">
          ...
          <div className="text-xs text-slate-600 dark:text-zinc-400 relative z-10 pt-4 md:pt-0 font-medium">
            <p>© 2026 L'Étoile S.A. Tous droits réservés.</p>
          </div>
        </div>
