...
- Name: admin.spec.ts >> Admin — Référentiel Commun (Clients & Services) >> CRUD Services : création d'une prestation standard et modification
- Location: tests\e2e\admin.spec.ts:58:7
...
Call log:
  - waiting for getByRole('button', { name: 'Services', exact: true })
...
> 62  |     await page.getByRole('button', { name: 'Services', exact: true }).click();
  ...
  111 |     await page.goto('/services');
  112 |     await expect(page.getByRole('button', { name: /Nouveau service/i })).toBeHidden();
  113 |   });
  114 | });
  115 | 
```