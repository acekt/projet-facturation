...
function getSessionSecret(): string {
  ...
  if (
    ...
    (!process.env.NODE_ENV && process.env.VITEST !== "true")
  ) {
...
const STATIC_ASSET_REGEX =
  /\.(?:png|jpg|jpeg|gif|webp|svg|ico|css|js|woff|woff2|ttf|otf|map)$/i;
...
export async function middleware(request: NextRequest) {
  ...
  const isPublicAsset =
    ...
    (STATIC_ASSET_REGEX.test(pathname) && !pathname.startsWith("/api"));

  // Validation de la session
  const session = sessionCookie
    ? await getSession(sessionCookie.value, SESSION_SECRET)
    : null;
  ...
  if (isSessionValid && session) {
    ...
    const isAdminOnlyRoute = ADMIN_FRONTEND_ROUTES.some(
      (route) => pathname === route || pathname.startsWith(route + "/"),
    );
    const isAdminOnlyApi = ADMIN_API_ROUTES.some(
    ...
    if (role !== "admin") {
      if (isAdminOnlyRoute) {
        return NextResponse.redirect(
          new URL("/?error=user_restricted", request.url),
        );
      }
      if (isAdminOnlyApi) {
        return new NextResponse(
