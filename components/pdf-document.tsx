...
export const PDFDocument = ({ document, type, settings }: PDFDocumentProps) => {
  ...
  const docTypeLabel =
    type === "devis" ? "DEVIS" : type === "facture" ? "FACTURE" : "AVOIR";
    ...
    <Document title={`${type.toUpperCase()}_${document.number}`}>
      <Page size="A4" style={styles.page}>
        ...
        <View style={styles.header}>
          ...
          <View style={styles.docTypeSection}>
            <Text style={{ fontSize: 10, marginBottom: 20 }}>
              Moanda, le {document.date}
            </Text>
            <View style={styles.docHeaderContainer}>
              <Text style={styles.docType}>
                {docTypeLabel}: N°{document.number}
              </Text>
            </View>
          </View>
        </View>
        ...
        <View style={{ marginBottom: 30, fontSize: 10 }}>
          ...
          <Text>
            Objet:{" "}
            {("subject" in document ? (document as any).subject : null) ||
              ("notes" in document ? (document as any).notes : null) ||
              "Prestations de services"}
          </Text>
          {type === "devis" &&
            "validUntil" in document &&
            (document as any).validUntil && (
              <Text>Date de validité: {(document as any).validUntil}</Text>
            )}
          <Text>NIF:</Text>
          <Text>BC:</Text>
        </View>
        ...
        <View style={styles.table}>
          ...
          {document.items.map((item, index) => (
            <View key={index} style={styles.tableRow}>
              ...
              <View style={styles.colPrice}>
                <Text>{formatCurrencyPDF(item.unitPrice)}</Text>
              </View>
              <View style={styles.colTotal}>
                <Text style={{ fontWeight: "bold" }}>
          ...
          <View style={styles.totalsContainer}>
            <View style={styles.totalCol}>
              ...
              <Text style={styles.totalVal}>
                {formatCurrencyPDF(document.subtotal)}
              </Text>
            </View>
            <View style={styles.totalCol}>
              ...
              <Text style={styles.totalVal}>
                {formatCurrencyPDF(
                  "discount" in document ? (document as any).discount : 0,
                )}
              </Text>
            </View>
            <View style={styles.totalCol}>
        ...
        <View style={styles.bankSection} wrap={false}>
          ...
          <View style={styles.bankGrid}>
            <Text style={{ fontSize: 8 }}>
              <Text style={{ fontWeight: "bold" }}>Banque:</Text>{" "}
              {settings.bankName}
            </Text>
            <Text style={{ fontSize: 8 }}>
              <Text style={{ fontWeight: "bold" }}>IBAN/RIB:</Text>{" "}
              {settings.iban}
            </Text>
          </View>
        </View>
        ...
        <View style={{ marginBottom: 40, fontSize: 10 }} wrap={false}>
          <Text style={{ fontWeight: "bold", marginBottom: 10 }}>
            Arrêter{" "}
            {type === "devis" ? "Le Présent devis" : "La Présente facture"} à la
            Somme totale de : ... FCFA TTC
          </Text>
          <Text style={{ color: "blue", fontSize: 9 }}>N.B:</Text>
          <Text style={{ color: "red", fontSize: 9 }}>
            - Les modes de règlement: *Espèces ; *Chèques ; *Virements .
          </Text>
          <Text style={{ color: "red", fontSize: 9 }}>
            - Les délais de règlement: *Au Comptant;
          </Text>
        </View>
        ...
        <View style={[styles.footer, { borderTopWidth: 0 }]}>
          <View style={styles.bottomCenter}>
            ...
            <Text>
              BP: {settings.address.split(",")[1]?.trim() || "111 LIBREVILLE"}
            </Text>
